...
static void ExchangedMethod(SEL originalSelector, SEL swizzledSelector, Class class) {
    ...
    Method oMethod = class_getInstanceMethod(class, originalSelector);
    Method sMethod = class_getInstanceMethod(class, swizzledSelector);
    
    BOOL dAMethod = class_addMethod(class, originalSelector, method_getImplementation(sMethod), method_getTypeEncoding(sMethod));
    if(dAMethod){
        class_replaceMethod(class, swizzledSelector, method_getImplementation(oMethod), method_getTypeEncoding(oMethod));
    }else{
...
- (void)s_layoutSubviews{
    ...
    for (UIView *childView in self.subviews) {
        ...
        for (UIView *sTabBarItem in childView.subviews) {
            //遍历UITabBarButton上的子视图
            //得到UITabBarButton 上的imageView Label  badgeView
